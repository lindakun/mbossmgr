...
<!-- 分页控件Form -->
<form id="pagerForm" method="post" action="host/midware/queryList">
	<!-- 当前页 -->
	...
	<div class="pageHeader">
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					<td><select id="hostMidwareSelect" class="combox" name="keyName">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="host/midware/toAdd" target="dialog"
				maxable=false mask=false resizable=false drawable=true
				title="[添加中间件]" width="800" height="480"><span>添加</span> </a>
			</li>
			<li><a class="delete" href="host/midware/delete?id={hostMidware_id}"
				target="ajaxTodo" title="确定要删除吗?"><span>删除</span> </a>
			</li>
			<li><a class="edit" href="host/midware/toEdit?id={hostMidware_id}"
				target="dialog" maxable=false mask=false resizable=false
				drawable=true title="[编辑中间件]" width="800" height="480"><span>修改</span>
			</a>
			</li>
			<li class="line">line</li>
			<li><a class="icon" href="host/midware/exportData"
		...
		<tbody>
			<c:forEach items="${hostMidWares}" varStatus="i" var="hostMidware">
				<tr target="hostMidware_id" rel="${hostMidware.MWID}">
					...
					<td><a href="host/midware/delete?id=${hostMidware.MWID}"
						target="ajaxTodo" title="确定要删除吗?" class="btnDel">删除</a>
						<a href="host/midware/toEdit?id=${hostMidware.MWID}"
						target="dialog" maxable=false mask=false resizable=false
						drawable=true title="[编辑中间件]" width="800" height="480"
						class="btnEdit">修改</a>
					</td>
				</tr>
			</c:forEach>
	...
	<div class="panelBar">
		<div class="pages">
			<span>显示</span> <select id="hostMidwarePageSelect" class="combox" name="numPerPage"
...
<script type="text/javascript">
	$(function() {
		//初始化每页显示页数选择框
		$("#hostMidwarePageSelect").val(fromCurNav("#numPerPage").val());
		//提交表单
		...
		if ($.string.isBlank(defKeyName)) {
			fromCurNav("#keyWord").attr("name", $("#hostMidwareSelect").val());
		} else {
			$("#hostMidwareSelect").val(defKeyName);
		}
		//添加条件查询选择框事件
		$("#hostMidwareSelect").change(
				function() {
