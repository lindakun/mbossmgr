...
<form id="pagerForm" method="post" action="host/main/queryListForLookup"
	...
	<div class="pageHeader">
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					<td><select id="lookuphostMainSelect" class="combox"
						name="keyName">
...
<div class="pageContent">
			...
			<c:forEach items="${hostMains}" varStatus="i" var="hostMain">
				<tr target="hostMain_id" rel="${hostMain.MAINID}">
					...
					<td><a class="btnSelect"
						href="javascript:$.bringBack({MAINID:'${hostMain.MAINID}', NAME:'${hostMain.NAME}', orgNum:'1001'})"
						title="查找带回">选择</a>
					</td>
				</tr>
			</c:forEach>
	...
	<div class="panelBar">
		<div class="pages">
			<span>显示</span> <select id="lookuphostMainPageSelect" class="combox"
				name="numPerPage"
...
<script type="text/javascript">
	$(function() {
		//初始化每页显示页数选择框
		fromCurDlg("#lookuphostMainPageSelect").val(
		...
		if ($.string.isBlank(defKeyName)) {
			fromCurDlg("#keyWord").attr("name",
					fromCurDlg("#lookuphostMainSelect").val());
		} else {
			fromCurDlg("#lookuphostMainSelect").val(defKeyName);
		}
		//添加条件查询选择框事件
		fromCurDlg("#lookuphostMainSelect").change(
				function() {
					fromCurDlg("#keyWord").attr("name",
